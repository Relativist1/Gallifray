...
class sym_gauss(object):
    ...
    def __init__(self,I0,sigma,phi, fov,dim=512,x_off=0, y_off=0):
    ...
    def vis_data(self, fov, uv='default',A=-0.5,interp=None,points=512):
        ...
        y0 = np.sin(self.phi)
        rho = np.sqrt(u**2 + v**2)
        a1 = 0.5*(1/self.sigma)**2
        b1 = 0
        c1 = a1
        exp1 = self.I0*np.exp(2*1j*np.pi*(u*x0+v*y0))
...
class asym_gauss(object):
    ...
    def __init__(self,I0,sigma, A, phi, fov,dim=512,x_off=0, y_off=0):
        """Creates a Symmetric Gaussian Model.
    ...
    def vis_data(self, fov, uv='default', A=-0.5,interp=None,points=512):
        ...
        y0 = np.sin(self.phi)
        rho = np.sqrt(u**2 + v**2)
        a1 = 0.5*((np.cos(self.phi)/self.s_maj)**2 + (np.sin(self.phi)/self.s_min)**2)
        b1 = 0.5*(np.sin(2*self.phi)/self.s_maj**2 - np.sin(2*self.phi)/self.s_min**2)
        c1 = 0.5*((np.sin(self.phi)/self.s_maj)**2 + (np.cos(self.phi)/self.s_min)**2)
        exp1 = self.I0*np.exp(2*1j*np.pi*(u*x0+v*y0))
