...
class xsringauss(object):
    ...
    def __init__(self,I0, R_p, R_n, ecn, f, gax, aq, gq, phi,fov,dim=512,x_off=0, y_off=0):
        ...
        if not 0<ecn<1:
            raise Exception("value of eccentricity must be between 0 and 1")
        if not 0<f<1:
            raise Exception("value of f must be between 0 and 1")
        if not 0<gq<1:
            raise Exception("value of gq must be between 0 and 1")
        """
        self.I0 = I0
        self.R_p = R_p
        self.R_n = R_n
